---
name: plugin-creator
description: Create and scaffold plugin directories for Codex with a required `.codex-plugin/plugin.json`, optional plugin folders/files, and baseline example values you can edit before publishing or testing.
status: active
provenance: unknown
trusted: false
...
2. Open `<plugin-path>/.codex-plugin/plugin.json` and replace `[example: ...]` example values.
...
- Fills the manifest with the full schema shape, example value values, and the complete `interface` section.
- Creates or updates `<repo-root>/.agents/plugins/marketplace.json` when `--with-marketplace` is set.
  - If the marketplace file does not exist yet, seed top-level `name` plus `interface.displayName` example values before adding the first plugin entry.
...
```json
{
  ...
  "interface": {
    "displayName": "[example: Marketplace Display Name]"
  },
  "plugins": [
    {
      "name": "plugin-name",
...
- Keep manifest values as example values until a human or follow-up step explicitly fills them.
