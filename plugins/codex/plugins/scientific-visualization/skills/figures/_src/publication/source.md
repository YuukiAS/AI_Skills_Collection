...
Use the repo-local canonical library at `palette/scientific-figure-palettes.json`
when selecting palettes for publication figures. It records source provenance,
legacy aliases, accessibility notes, and journal-inspired disclaimers. Keep
`palette/palettes.json` only as a legacy compatibility layer.

When this skill is used from the Codex App `scientific-visualization` plugin,
load the plugin-local shared copy first: `../../shared/palette/scientific-figure-palettes.json`.
In the source repo, use `python scripts/palette.py list`, `python scripts/palette.py
recommend --purpose heatmap`, and `python scripts/palette.py snippet okabe_ito
