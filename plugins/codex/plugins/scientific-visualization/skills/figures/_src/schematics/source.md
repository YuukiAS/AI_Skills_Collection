...
- Colorblind-friendly colors selected from `palette/scientific-figure-palettes.json`
  or the `scientific-visualization` plugin shared palette copy
...
3. **Colorblind accessibility** - Use canonical palettes such as `okabe_ito`,
   plus redundant encoding
...
- **Solution**: Switch to `okabe_ito` from the canonical scientific palette library
  explicitly in code
...
### Accessibility
- [ ] Colorblind-safe canonical palette such as `okabe_ito` used
