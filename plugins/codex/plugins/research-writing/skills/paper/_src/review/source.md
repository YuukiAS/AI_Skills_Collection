...
license: MIT base skill plus locally rewritten ICLR workflow from no-license-observed source; see provenance
metadata:
...
Use this mode when the user asks for paper验收, pre-submission self-review,
reviewer-style critique, likely reviewer objections, acceptance risk, top-conference
readiness, ICLR review, or rebuttal analysis. The review must be evidence-led:
every important judgment should point to a manuscript section, figure, table,
equation, page, or explicitly unavailable artifact.

Workflow:

1. Build a manuscript map before scoring: problem, gap, method, assumptions,
   central claims, experiments, limitations, contribution, and nearest prior
   work.
2. Run independent review lenses for best-supported contribution, critical
   blocker, method soundness, evidence/experiments, novelty/positioning,
   clarity, and ethics/reproducibility. Add no more than three domain lenses
   when the paper needs specialist judgment.
3. Audit each central claim as `supported`, `partially_supported`,
   `unsupported`, or `not_verifiable`. Missing evidence and contradictory
   evidence are different failure modes.
4. Score roles independently, then synthesize as an area-chair style judgment.
   Do not average scores; a fatal soundness or evidence blocker cannot be
   canceled by good writing, topical fit, or a single enthusiastic lens.
5. Separate fatal blockers, major concerns, minor concerns, and strengths. Keep
   strengths only when the manuscript itself supports them.
6. For each requested revision, state the exact change or experiment, linked
   concern, expected review impact, plausible score movement, and remaining
   uncertainty.
7. In rebuttal mode, track each reviewer concern as `resolved`,
   `partially_resolved`, `unresolved`, or `unknown`. Reviewer silence is not
   agreement, and a promised future experiment is not completed evidence.

Report skeleton:

- Verdict: area-chair style rating, confidence, rationale, and primary blocker
  or acceptance condition.
- Manuscript map.
- Independent reviewer table with role, rating, confidence, main reason, and
  evidence anchor.
- Strengths.
- Concern ledger with stable IDs, severity, evidence, why it matters, and
  required resolution.
- Claim-evidence audit.
- Questions whose answers could change the score.
- Prioritized revision plan.
- Rebuttal assessment when comments or responses are supplied.
- Citation audit, with inaccessible claims marked `not_verifiable`.

Source distilled from `Haoran-98/ICLR-reviewer` at
`3a105ae9270f`; no license file was observed in the clone, and the integration
uses locally rewritten workflow rules per user instruction. See
`docs/provenance/WRITING_SKILL_REPOS_2026_07_28.md`.

## When to Use This Skill

This skill should be used when:
- The user asks whether a manuscript is ready, acceptable, defensible, or safe to submit.
- The user asks to check a paper like a reviewer without naming a specific venue.
- The user asks whether results support the claims, whether limitations are over/understated, or how reviewers may attack the work.
...
Do not generate diagrams just because a manuscript is being reviewed. Reviewer-style acceptance checks should usually stay textual unless the user asks for a visual summary or the review would clearly benefit from a claim-evidence table, concern matrix, rebuttal map, or figure-to-claim diagram.

Route visual work only when it is part of the requested deliverable:

- Use tables for reviewer roles, concern ledgers, claim-evidence audits, and rebuttal status.
- Use `markdown-mermaid-writing`, `drawio-diagrams`, or `d2-diagrams` for complex rebuttal dependency maps or method/claim flow diagrams.
- Use presentation/poster skills only when the user asks for slides, a talk, group meeting material, or a visual summary.
