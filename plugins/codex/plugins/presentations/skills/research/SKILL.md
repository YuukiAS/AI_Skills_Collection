...
- Choose the presentation format from the user's deliverable: PPT, PowerPoint, `.pptx`, editable, Slides, Google Slides, or "I need to edit it later" means editable Presentation/Slides; Beamer, LaTeX slides, `.tex`, academic PDF, or a venue/project-locked TeX template means Beamer/LaTeX; outline/storyline-only requests can stop at the deck plan.
- In the `presentation-desktop` profile, an unspecified group meeting, research update, paper talk, journal club, seminar, defense, methods/results update, or "research slides" request defaults to the exact CUHK Beamer route: source-editable `.tex` from `../../shared/templates/cuhk/beamer/source/` plus rendered PDF.
...
6. Decide the format from the user's requested deliverable:
   ...
   - Group meeting, research update, paper talk, journal club, seminar, defense, methods/results update, or research slides in a desktop presentation context with no format specified -> exact CUHK Beamer route, using source-editable `.tex` plus rendered PDF.
   - Outline, storyline, or page-by-page plan only -> stop at `deck-plan.yaml`.
   The normal one-call exact-CUHK production entry is `../../shared/scripts/generate_research_presentation_production_entry.py --input-bundle <bundle.json> --out-dir <output-dir>`. It reads a file/path-oriented research bundle, creates the source-fidelity map and deck plan, performs normal gold selection, consumes Stage 3 layouts, renders `.tex -> PDF -> PNG`, and writes task-local visual-review inputs. Do not call benchmark generators as the production entrypoint.
   The official existing-deck revision completion gate is `../../shared/scripts/validate_existing_deck_revision_entry.py --revision-packet <packet.json> --out <summary.json>`. Use it for targeted revisions of a reviewer-seen deck after the revised source has been rerendered. It consumes the reviewer-seen baseline, accepted-element ledger, targeted feedback, rerender evidence, high-resolution problem pages, first-use dependency checks, rendered scientific-object QA, English final pass, and independent visual review. It may close only as `PASS_REVIEWED`; otherwise it returns `REVISE` or `BLOCKED`.
...
8. Preserve equations in LaTeX inside the deck plan. Before compiling `.tex` or producing Beamer/PDF, invoke the locally installed `render-chinese-math-pdf` skill. Use that skill to probe for the LaTeX compiler, TeX packages, font availability, writable TeX caches, local render resources, and PDF QA tools instead of assuming `xelatex`/`lualatex` paths. On the YuukiAS workstation, preserve `/home/yuukias/render_resources/chinese_math_pdf` as the current TRACE-compatible render resource location when the probe reports it. If the local skill is not installed or any required resource is missing, block and report the exact missing dependency for user installation rather than substituting a retired server-only path.
9. Use the CUHK default template when no stronger project, course, company, or conference template is specified. For the default academic/research route and for exact CUHK Beamer reproduction, use `../../shared/templates/cuhk/beamer/source/` as the canonical source; the title slide layout is locked and only content fields such as title, subtitle, author, institute, and date may change. `../../shared/templates/cuhk/design-tokens.json`, `../../shared/templates/cuhk/beamer/main.tex`, and `../../shared/templates/cuhk/pptx/` are derived/non-exact scaffolds and must not be treated as the exact production source.
10. After file creation, render the deck to PDF/images and run scientific visual QA. The normal one-call route also writes a deck contact sheet, sequence summary, and bounded quality-loop state so an independent reviewer can judge deck-level rhythm instead of aggregating page PASS results. A deck is not `complete` merely because a file exists or mechanical checks pass. For editable PPTX regression, the PDF/images must come from the PPTX through a real presentation engine such as `soffice`, `libreoffice`, or an explicitly configured renderer. For English research slides, run a scientific-prose final pass only after scientific structure, formulas, claims, and citations are frozen; then rerender before final QA.
...
13. For real-world deck revisions or user/advisor feedback loops, read `references/real-world-presentation-guardrails.md` after this file. It is the runtime entry for mature cross-project revision rules; source-only maintenance TODOs and project-specific histories stay outside ordinary installed plugin payloads. Natural requests such as "继续按这些批注返修我现有的组会PPT", "这个版本其他页别动，只修我指出的页面", or "保留上轮接受的布局，继续改剩下的问题" are existing-deck revision requests, not authorization to regenerate a new deck from scratch.
...
Deck-level QA must also inspect a real contact sheet or equivalent full-sequence object for adjacent density jumps, repeated layout face, overfull/empty pages, redundant filler, result-to-failure-to-next-experiment rhythm, workstream transition balance, and title/object/formula/image alternation. The top-level review package PASS is not enough without item-level judgement and observations for the deck-level object.

The generator must not assign final `PASS`. It may write PPTX, raw assets, evidence manifests, contact sheets, quality-loop state, and render status. An independent scientific visual reviewer must read the plan/evidence/rendered PNGs/reference IDs/contact sheet before assigning `PASS`, `REVISE`, or `BLOCKED`. If real PPTX rendering is unavailable, status is `BLOCKED_REAL_PPTX_RENDER`; do not reconstruct a parallel PDF and claim it represents the PPTX.
