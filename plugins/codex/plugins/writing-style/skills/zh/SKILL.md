---
name: chinese-prose
description: 中文报告、README、Markdown/PDF 成稿、技术文档、科研说明、组会材料和“说人话”终审。用于中文为主、降低 AI 味/翻译腔/模板腔/宣传腔、移除非必要英文、修正版本/阶段名误用和机器字段先行，同时保护事实、数字、术语、命令、引用、实验结果、证据边界和中文读者习惯。
status: active
provenance: local
trusted: true
...
用于组会汇报、口头汇报、汇报提纲、可以直接讲的版本、面向老板/作者/科研负责人的中文材料。

- 开头先给 30 秒版本：有没有新结果，最重要指标是什么，相比上次多做了什么，哪些路线有用，哪些路线失败，下一步做什么。
- 按听众问题组织，而不是按文件、线程、审计表或代码路径组织。
- 主讲部分用短 bullet、小表格和“可以这样讲”句子。证据路径、commit、branch、命令和日志放到最后的备份/证据区。
- 把内部英文普通词翻成中文，除非它是必须精确匹配的指标、模型名、字段名、路径或命令。
- 明确证据边界：本地信号、还不是官网结果、不能直接提交、需要更多折复现、只作备份、需要作者拍板。
- 失败路线不能只写“失败”。说明失败表现、可能原因、是否仍有价值，以及当前决策：继续、暂停、备份或先排查流程。
- 最终检查：第一页能否直接投屏，30 秒版本能否直接念，读者能否在 1 分钟内知道下一步做什么。
