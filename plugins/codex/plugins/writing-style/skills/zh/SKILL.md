...
先判断英文表达属于哪一类，再决定是否保留。算法名、数据集名、模型名、指标名、变量、公式、标准正式术语、论文题名、代码标识、路径、字段和为检索或精确定位必须保留的 identifier，应保持原样或首次解释后保留。除此之外，如果英文只是 `A + B`、`A vs B`、`A -> B`、连字符或斜杠堆叠的 noun-stack、实验流程标签、工作流标签、内部分类标签，就不能让它继续承担中文标题、段落骨架或句子主干。

这不是禁词表，也不是逐词翻译表。处理这类非正式英文关系标签时，要先说明它在当前句子里的关系：谁保持不变，谁被训练；谁和谁比较；一个步骤如何进入下一个步骤；哪个条件被改变；现有证据还能或不能说明什么。比如 `shared encoder + linear head` 这类写法不应原样放进中文正文，而应写成“共享编码器保持不变，只训练线性输出层”；`training data -> model update` 这类箭头链应写成“先核对训练数据，再更新模型”。如果同一段还包含 `FedFisher`、`LoRA`、`Dice` 这类正式算法、方法或指标名称，它们应保留。
...
尤其要警惕连字符英文复合名词、斜杠堆叠和 `X aggregation 与 Y approximation` 这类英文科研句法。它们看起来像术语，实际常常只是把普通概念压缩成英文名词串。除非它是论文题名、算法正式名称、表格中必须对齐的字段或代码标识，否则要拆开讲清楚：谁在什么参考点附近聚合什么信息、近似什么目标、和另一种做法差在哪里。不要让非正式英文 noun-stack 直接出现在中文正文里承担主谓宾。
...
也不要让仓库维护和审计过程替代科学叙述。面向读者的科研正文默认描述“研究事实是什么、数据说明什么、当前实验能证明什么”，不要把仓库、manifest、文件状态、实现状态或审计记录写成正文主语。若文件名、路径或字段确实是必要复核定位，应放在括号、脚注、方法定位或技术附录；正文主体仍要先说它代表的科学事实、数据边界、实验定位或未验证条件。
