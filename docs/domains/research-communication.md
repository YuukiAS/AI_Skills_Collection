...
- `project-thread-handoff` (`skills/science/communication/project-thread-handoff`): Use only after explicit Skill selection to hand off the current project thread or recover a same-Project old thread; do not use for ordinary summaries, status checks, brainstorming, or general continuation.
