...
```bash
ai-skills install --target repo --domain ai-ml --mode symlink --write-agents-md
```

Install a few skills precisely:

```bash
ai-skills install --target repo --skill tool/ai-ml/modal --skill tool/ai-ml/pufferlib --skill tool/ai-ml/pytorch-lightning --mode symlink --write-agents-md
```
