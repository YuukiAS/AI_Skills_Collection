...
| Broad AI research engineering catalog | `Orchestra-Research/AI-Research-SKILLS` | Record as source catalog only for now; do not import the 90+ training, fine-tuning, serving, RAG, MLOps, and agent-framework tool skills until a focused AI/ML engineering plugin/profile is approved. |
| Uncloneable Paper Craft source | `zsyggq/paper-craft-skills` | Record only because anonymous GitHub clone and `git ls-remote` return repository not found; revisit only if the user provides a public or corrected URL. |
| Unlicensed ICLR reviewer source | `Haoran-98/ICLR-reviewer` | Resolved by explicit user instruction on 2026-07-28: locally rewritten workflow rules were merged into `peer-review`; no standalone upstream skill or source text imported, and missing license status remains recorded. |
