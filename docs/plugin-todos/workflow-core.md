...
1. **Freeze the acceptance-artifact specification before bulk generation.** Record the review object, source range, candidate-output range, citation/reference inclusion rule, target format, semantic page/slide granularity, and whether side-by-side/top-bottom comparison is required. Do not start a large PPT/PDF from an implicit layout guess.
...
3. **Align comparison by semantic unit.** Each page/slide compares the same section, paragraph group, table, formula, code block, or other coherent unit. Do not mechanically split into sentence/half-sentence pages. If the candidate reorders the source, build an alignment map/manifest before generating the comparison artifact.
4. **Readability outranks page count.** More pages are acceptable only when they improve legibility and correspondence. Hundreds of pages are not evidence of completeness if individual pages are sparse, misaligned, or difficult to compare.
5. **Respect explicit citation/reference exclusions.** If the user asks to exclude citations/references, keep them out of the main comparison surface and state that they remain available in the canonical source rather than flooding the acceptance artifact.
6. **Final user-facing artifacts must use the canonical repo path.** `/tmp` is intermediate-only. Final acceptance artifacts must live under the task's repo-owned `private/exports/...` or another frozen repo-local delivery path, and handoff must give that canonical locator.
7. **PASS needs minimum real-open/content validation.** At minimum verify the artifact opens, expected page/slide count, major section coverage, key pages, and obvious clipping/misalignment. If every page was not visually checked, state `ALL_PAGES_VISUALLY_CHECKED=NO`; do not imply full visual QA.
8. **User feedback is not a normal QA loop.** After the first material acceptance-artifact failure, fix the generator/alignment/spec mechanism before regenerating. A second same-class failure triggers the existing repeat-failure circuit breaker: re-check input identity, alignment strategy, delivery spec, and actual review surface before another bulk run.
9. **Artifact-only repair should minimize cost.** If source/final candidate/G7/rubric do not change, keep repair to layout/alignment/packaging; do not rerun the model or paid review. Before a large regeneration, generate a small representative section/sample and self-check it first.
10. **Handoff must name the artifact truthfully.** Distinguish complete candidate, comparison deck, gate evidence, render preview, and QA manifest. Do not call a preview page the acceptance artifact, and do not claim PPT/Office render QA merely because a same-source PDF rendered successfully.
