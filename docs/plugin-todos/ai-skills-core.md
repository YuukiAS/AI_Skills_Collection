...
### Establish repository 5.0 release epoch with independent plugin SemVer and changelogs
status: PROMOTE_NOW
...
candidate action: publish the next repository-level maintenance epoch as `5.0.0`; add one repository-version source of truth; keep existing plugin versions monotonic from the `4.4.2` legacy lockstep baseline rather than resetting downward; allow affected plugins to bump independently; create `docs/plugin-changelogs/<plugin>.md` for all ten central plugins; make root CHANGELOG the repository release homepage and show plugin version/status/changelog links in README.
promotion gate: version/changelog/README/generated-payload consistency tests + real install/upgrade smoke; maintenance changelogs remain source-only; same-name plugin versions must not move backward.
...
### Release SemVer is separate from capability status
status: PROMOTED
...
- Do not bump all ten plugin versions merely because the repository publishes 5.0.0.
- Do not reset existing same-name plugin versions downward to `1.0.0` / `0.x`.
- Do not fabricate detailed per-plugin changelog history before the 4.4.2 legacy lockstep baseline.
