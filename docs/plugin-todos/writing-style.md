...
evidence: `YuukiAS/TRACE` commits `e36cb5d93fc882ce158d88ac9201fe494b98b69a` and `1de90f2f26b3f787073ecedd7a4df41a985712eb`; v4 exposed repeated `Failure prevented` / mechanical `Example.` language, while the 32-page v5 still contains reader-facing labels such as `What it measures` and `Focal method` that the user found machine-like or needlessly indirect even after the presentation English final-pass step
problem: 最终英文 scientific slide 仍可能保留模板化标签、内部分类腔和不自然 microcopy。v5 说明即使 presentations 已要求 scientific-prose final pass，也不能只检查语法和明显 AI 模板句；还要判断一个 label 是否真的是研究者会对听众使用的最直接说法，例如优先用 `Purpose`、直接写方法名或 `Methods compared`，而不是为结构化而创造抽象标签。需要先确认这是 current `scientific-prose` 本身的 slide-microcopy coverage 缺口，还是 presentation handoff 提供的上下文/验收不够。
project-specific context: CAT-TRACE、TRACE、CORAL、catalogue/open-tail、具体 metric 和具体 slide 句子属于当前项目；不能为这个 deck 建专用禁词表，也不能让 writing-style 改动公式、模型结构、页面布局或科学 claim。
