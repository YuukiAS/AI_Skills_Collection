# 056 交付工作流可靠性基线 — Implementation Plan v0.4
...
- AI_Skills planning baseline：main@72f163330ea5a21637df95f08289e2c4739d2bd9
...
- Base architecture：Product Delivery Discipline v6 + post-probe addendum
...
本 Plan 替代 v0.3 execution package。它不是 v7，不创建 successor，不执行 production。只有独立 Critic 对同一 v0.4 Plan + Goal + Kickoff 给 READY_FOR_CODEX=YES 后，用户实际发送 approved Kickoff，才授权 Executor 开始。
...
## 3. W2 v0.4 bounded amendment：least-privilege equivalent recovery
...
不在 056 新增 generic authorization renderer。当前 v1.4 Planner/Critic execution package 已负责自带 Kickoff；Bridge 0.8.1 已负责 runtime consumption。普通非-Planner/Critic复杂任务的 copy-ready kickoff 留作 workflow-core 后续 refinement。
...
## 5. Write scope after approved Kickoff

### AI_Skills_Collection — WRITE

仅在 approved Kickoff 后创建/使用历史任务兼容 branch：

reviewed/056_product_delivery_discipline

task-owned worktree 建议继续使用：
...
必须保护 0.8.4 semantic task-key behavior与 0.8.0/0.8.1 Persistent Run/upfront authorization。
...
| Gate | 证明的能力 | v0.4 主要 evidence | 失败条件 |
|---|---|---|---|
...
当前 OpenAI Codex source 继续支持原 post-probe判断：Default mode required input 应使用 concise plain-text question；request_user_input 不用于 permission escalation。
...
本 v0.4 execution package 仍不授权：

- 修改真实用户 CODEX_HOME；
- real Host Policy install/update；
- final real-user W2/G1 fresh-session smoke。

source/unit/isolated fixture可以测试 Host逻辑。最终真实 Host integration若仍是 release-critical requirement，必须在 implementation review 后单独获用户授权，不能用 isolated evidence 冒充。

## 10. Validation order

1. source/identity/version preflight；
2. focused cheap deterministic regression；
3. original W1-W5 / least-priv / Frontend / Maintainer / Bridge focused tests；
4. source-generated Marketplace parity；
5. broad/full repository regression；
6. candidate plugin replay / normal-entry evidence；
7. freeze exact candidate tuple；
8. independent implementation review；
9. 后续如需 real Host integration，再单独授权。
