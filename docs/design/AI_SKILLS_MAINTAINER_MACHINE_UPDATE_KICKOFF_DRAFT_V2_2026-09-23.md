# AI Skills Maintainer machine update orchestration — Kickoff Draft V2
...
Implement the Critic-approved V2 capability so the normal user workflow becomes:
...
This task may consume/read/test the current canonical Bridge implementation and may, after explicit current-user authorization in the final kickoff:
...
- update a **minimal Bridge-side pointer/documentation line** only if the approved design requires a Bridge-only release task to discover that formal distribution closure is owned by AI Skills Maintainer.
...
The implementation must pass the V2 Gate Matrix exactly as frozen in Proposal V2:
...
If the final implementation scope remains V2 and G1–G5 pass:

- repository `5.0.7 -> 5.1.0`;
- `ai-skills-core 0.4 -> 0.5`;
- `workflow-core`: no bump;
- domain plugins: no bump;
- Bridge Kit: no bump unless Bridge production behavior changes.
