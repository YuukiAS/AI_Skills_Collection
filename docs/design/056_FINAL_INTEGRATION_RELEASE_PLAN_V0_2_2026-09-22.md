# 056 交付工作流可靠性基线 — Final Integration / Release Plan v0.2
...
- Status: `READY_FOR_INDEPENDENT_CRITIC_REVIEW_V0_2`
...
## 0. v0.2 amendment — ACCEPT C056-F1-APPROVED-KICKOFF-SELF-BLOCKS

Planner disposition: **ACCEPT**.

The v0.1 integration/release design is unchanged. The only defect was execution-authorization presentation: the exact Kickoff that the user would send after Critic PASS still carried `Status: DRAFT_NOT_AUTHORIZED`, while the Canonical Goal carried the same marker. That can cause an Executor to treat the approved current-user message as still unauthorized and search for an additional repo-local Critic PASS artifact.

v0.2 changes only the consumer contract:

- Canonical Goal status becomes `READY_FOR_USER_AUTHORIZATION`.
- Kickoff status becomes `READY_FOR_USER_AUTHORIZATION`.
- Critic PASS remains review approval only; it does not execute anything.
- Once the user sends the exact approved v0.2 Kickoff as the current user message, that message itself is the bounded authorization for the effects enumerated in the Kickoff.
- The Executor must not require an additional repo-local Critic PASS artifact and must not treat planning/review metadata elsewhere in the repo as a second authorization gate.
- Only genuinely new target, scope, destructive effect, provider/account/credential purpose, or cost requires fresh user authorization.
- No PASS receipt schema, authorization database, state machine, ledger, watcher, Gate, production behavior, version route, merge strategy, Host scope or branch-cleanup rule is added or changed.

Everything from Sections 1 onward remains the v0.1 integration/release contract.
