# AI Skills Maintainer machine update orchestration — Canonical Goal V2.1
...
The canonical producer contract lives in AI Skills Maintainer.

A **mandatory minimal owner locator** must be added to `YuukiAS/GPT_Codex_AI_Bridge_Kit/AGENTS.md` during the later authorized implementation. That locator must make the Bridge formal-release handoff discoverable from the Bridge repo's normal maintenance entry while preserving these boundaries:

- Bridge runtime / Host / Lite / Review / Control / Persistent Run implementation stays owned by Bridge Kit;
- formal Bridge distribution/version closure, including the moving `release` ref, is owned by AI Skills Maintainer -> internal `bridge-kit-maintainer`;
- a Bridge formal release must hand off to that owner before distribution completion;
- the full producer contract remains only in AI_Skills_Collection and is not copied into Bridge.

Do not duplicate the locator into Bridge README/QUICKSTART/CHANGELOG as a second policy.
...
Use exactly G1–G5 from Proposal V2.1:

- G1 — short request + formal release/update-impact routing;
- G2 — legacy main->release bootstrap + install/self-update + fresh session;
- G3 — Bridge Kit maintenance + Bridge AGENTS producer-owner routing + canonical Bridge delegation, including exact formal-commit eligibility, fast-forward-only release advancement, remote-ref verification, post-release docs/TODO non-advancement, and fail-closed inconsistent/non-fast-forward cases;
...
1. independent Critic PASSes this V2.1 package; and
