# Project Thread Handoff — Codex Kickoff Draft v0.1
...
- explicit-only；
- `policy.allow_implicit_invocation: false` 只在 `agents/openai.yaml`；
...
G1：
Installation / Explicit Invocation Boundary。
...
记录 SHA-256 与 file list 到 task MANIFEST。不要只留 `/tmp`，且 G1/G2 完成前不要清理 worktree/export。

### 7. Version / README

当前 version policy 下，本 Goal 冻结 repository release class：

`MINOR`

planning baseline 预期：
...
当 final candidate 已完整、push、远端验证并且只剩 target ChatGPT regular Chat acceptance 时，停止在
`FINAL_CANDIDATE_READY_FOR_TARGET_ACCEPTANCE=YES`。

不要把 source/tests/zip/G3/Codex smoke 写成
