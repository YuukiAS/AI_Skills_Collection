你现在执行 AI Skills Maintenance Board central implementation v0.3。只执行已经通过 v5 design review 并冻结在 v0.3 Plan / Goal 中的 central stage；不要重新设计 lifecycle。
...
只有当独立 Critic 对这套 v0.3 package 给出 READY_FOR_CODEX=YES，且我随后实际发送这段 Kickoff，才形成当前执行授权。
...
5. 做一次完整 current-inbox bootstrap：
   ...
   - PROJECT_LOCAL / REJECTED / SUPERSEDED / historical resolved 不机械上板，但必须在 coverage evidence 里明确记录
   - 创建：
...
6. tracking Issue / Project 是用户直接看的 artifact。
   每个 tracking Issue：
   - title 必须自然、简洁
   - 顶部固定：
     问题：
...
7. 只要你创建或实质修改 Kanban / Project 的 reader-facing copy，必须在 GitHub mutation 前真实调用当前安装的 Clear Writing（writing-style）。
   Clear Writing 不得改变 maturity、Status、Area、required-consumer truth、exact locators、Resolution commit 或 evidence meaning。
...
9. canonical board policy 只能完整存在于：
...
10. no-tool surface 必须保留 semantic ownership。
    如果 Planner/Critic/GPT 当前不能修改 GitHub Project：
    - 更新自己能合法更新的 Issue evidence/summary
    - 输出 exact pending Project mutation
    - 不得声称已经同步
    - 不得要求用户手工拖 Kanban
    下一 Project-capable maintenance action 应先核对 mutation 仍属于当前 tracking Issue、evidence 仍 current，再机械应用。

11. 本 board task 自身 tracking Issue 在 central implementation 期间保持 DOING。
    execution-ready Critic PASS 本身不进入 ADAPTING。
    Executor 自报完成也不进入 ADAPTING。

12. Stage A central implementation 完成后：
    - push exact task branch
    - 保存真实 Project/config/coverage/surface evidence
    - 停止等待独立 implementation Critic/Reviewer
    - Reviewer 必须检查真实 Project surface、repo diff、TODO_COVERAGE、BOARD-01、Clear Writing 结果和 role-consumer contract
    - Executor summary 不能替代 review

13. 独立 implementation Reviewer PASS 后，如果 latest main 没有语义冲突，本 Kickoff 已授权同一 central scope 内：
    - fetch latest main
...
14. main integration 后，必须给 AI Research Stack ChatGPT Project instructions 安装下面 exact short trigger/locator：
...
15. 如果 GitHub built-in Project workflow 配置仍只能通过 UI，而你没有受支持浏览器，可以在 backfill 前向我提出一次最小 HUMAN_ONLY GitHub UI 操作；完成后必须 read back 实际 workflow state，再继续同一 Goal。

16. central implementation complete 的条件是：
...
17. 进入 ADAPTING 时，只解析并冻结当前五个 required logical consumers 的 exact current identities/locators：
...
18. AI Skills Maintainer 只能是 per-current-consumer adaptation executor，不是 cross-machine controller。
...
19. 本 Kickoff **不授权**任何 server/local machine adaptation。
    当 central implementation complete、exact consumers 已冻结后：
    - 生成 exact per-consumer Maintainer handoff(s)
    - 等待对应 current-user machine/credential/remote authority
    - Project 保持 ADAPTING
    不得为了追求 DONE 静默访问机器或扩大 remote authority。

20. per-consumer Maintainer 如果有 Project mutation能力，可更新当前 consumer evidence/checklist；仍有 pending 就保持 ADAPTING。
    如果没有 Project mutation能力，必须输出 exact completion mutation，至少包括：
    - tracking Issue
    - consumer
...
21. 最终 DONE 不是本 initial central Kickoff 的执行范围。
    只有后续 tracking Issue 聚合确认五个 required consumers 全部 PASS/N/A，并有 durable evidence、Resolution commit、completed close，才能：
    ADAPTING -> DONE

    单台 Maintainer PASS 不得关闭 top-level Issue。

22. 本任务 central stage 明确禁止：
...
如果 exact worktree 无法合法创建、GitHub身份/权限不符、project scope 不可取得、Clear Writing unavailable、Project workflow无法验证、Project exact-title冲突、出现语义 main drift，停止并报告 exact blocker；不得换路径、换分支、扩大权限或静默降级。

最终在 central stage 不得声称 OVERALL_GOAL_ACHIEVED=YES。只有未来五 consumer aggregate closure 才能最终 DONE。
