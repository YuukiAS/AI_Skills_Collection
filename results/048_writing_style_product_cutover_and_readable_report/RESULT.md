...
- repaired stale Text Review blocker rounds by rebuilding the candidate from the stronger-goal model output, removing duplicate/truncated deterministic restore artifacts, deleting the unreadable literal-fragment appendix, and then removing the remaining repeated/truncated tail while restoring the engineering comparison as a single reader-facing method table;
