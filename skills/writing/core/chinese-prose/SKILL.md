...
英文技术词第一次保留时，必须让读者知道它在当前论证里做什么、为什么需要它。只展开缩写、只给括号翻译、或只说“这是技术术语”都不够。自然顺序通常是：先说有边界的中文科学判断，再给直观解释或具体关系，再保留必要的英文名、公式或正式符号，最后说明证据边界和限制。

`useful_recognition` 只能作为首用识别桥。一个非精确身份的英文表达如果已经通过中文解释建立了含义，后文通常应使用中文说法；不能因为论文里常见、听起来专业、或方便压缩关系，就反复用英文名词短语支撑中文句子。普通比较、推理、限定、转折、组织和下一步关系要用中文直接说明。

读者面对的句子要说清楚关系本身：比较了什么，哪个条件变了，结论受什么限制，下一步为什么自然跟上。不要用压缩标签替代关系，也不要用审计、状态、流程、辩护或“不是 X 而是 Y”的架子掩盖实际观察。必要时加一句短桥接说明；可读性不是把字数压到最低。
