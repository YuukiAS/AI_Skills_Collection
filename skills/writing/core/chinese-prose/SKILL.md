...
`{{...}}`、`[[...]]`、`<ref>`、HTML 注释、模板参数、公式代码块、
未加数学分隔符的 LaTeX 命令，或把表格当普通文本的管道行。需要保留的是
引用意义、作者归因、公式关系和表格内容，不是来源平台的脚手架。

简体/繁体是读者约束。用户要求简体时，普通正文默认用简体；用户要求繁体时，
普通正文默认用繁体。模型名、数据集、指标、变量、代码、路径、命令、字段名、
引用标题和必须精确匹配的英文 token 不做机械简繁转换。

普通英文和内部过程词要在中文实现阶段消化掉。`provenance`、`audit`、
`candidate`、`pipeline`、`scientific gap`、`reader effort`、
`state of the art` 这类词如果只是承载普通解释、流程或抽象关系，应改成
自然中文；只有当它们是原文必须保留的术语、标题、字段、代码或用户明确要求的
讨论对象时才保留。
