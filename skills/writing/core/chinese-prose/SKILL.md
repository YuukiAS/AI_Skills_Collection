...
同样要避免把科研证据边界写成法律或取证语气。除非原文引用、论文标题、文件名或审计协议必须逐字保留，不要在中文正文里留下 `forensic-level exact proof`、`legal proof`、`data contract` 这类说法；应改成“逐项可追溯的精确证据”“足以独立核验的训练记录”“数据使用与字段约束说明”等当前语境真正需要表达的中文含义。

联邦学习、医学影像和机器学习报告里也要区分“真正需要保留的英文术语”和“只是英文写作习惯”。在中文正文中，`client` / `clients` 通常应写成“参与方”“中心”或“医院”；`local` / `global` 通常写成“本地”“全局”或“汇总后”；`pooled` 通常写成“合并数据训练”“集中式训练”或“集中式适应”；`baseline` 通常写成“基线”或“对照方法”；`checkpoint` 在不是路径或文件名时通常写成“模型权重”或“权重文件”。这些词只有在论文标题、代码/配置字段、公式符号、路径、表格字段、引用原文，或首次解释后确需与外部术语对齐时才保留英文。
