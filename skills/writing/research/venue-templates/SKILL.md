...
description: This skill should be used when preparing manuscripts for journal submission, conference papers, research posters, or grant proposals and need venue-specific formatting requirements and templates.
status: active
provenance: unknown
trusted: false
...
This skill owns venue and template requirements, not automatic figure creation. Do not generate schematics just because a manuscript, conference paper, poster, or grant is being prepared. Add or revise visuals only when the venue requires them, the user asks for them, or the document has a clear figure/table/poster requirement.

For manuscript figures, top-conference plots, poster figure palettes, or publication color QA, route figure styling to `scientific-visualization`. Use `scientific-schematics`, `drawio-diagrams`, `d2-diagrams`, or `markdown-mermaid-writing` only for requested or necessary conceptual, workflow, architecture, data-flow, or experimental-design diagrams.
