...
For manuscript or top-conference figures, use the canonical scientific palette
library before choosing seaborn defaults. In this repo, run `python
scripts/palette.py recommend --purpose categorical` or `python scripts/palette.py
snippet okabe_ito --target seaborn`; inside the `scientific-visualization`
plugin, read `../../shared/palette/scientific-figure-palettes.json`.
