---
name: paper-2-web
description: This skill should be used when converting academic papers into promotional and presentation formats including interactive websites (Paper2Web), presentation videos (Paper2Video), and conference posters (Paper2Poster).
status: active
provenance: unknown
trusted: false
...
## Visual Routing for Converted Documents

This conversion skill should preserve and extract the source document; it should not invent new diagrams by default. Route to visualization, schematic, poster, slide, or Mermaid skills only when the user asks for a visual deliverable, the source contains a diagram that must be reconstructed, or the document has a complex mechanism/workflow that cannot be understood clearly in text alone.

When visuals are needed, keep them source-faithful: cite the source material, label speculative reconstruction as such, and avoid replacing extracted evidence with decorative images.
