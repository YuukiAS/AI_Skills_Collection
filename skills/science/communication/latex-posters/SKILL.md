...
- Use `palette/scientific-figure-palettes.json` or the
  `scientific-visualization` plugin shared palette copy as the canonical source.
- Use `python scripts/palette.py snippet <palette-id> --target latex` to generate
  `\definecolor` commands.
- Prefer `okabe_ito` for categorical poster figures, `viridis`/`cividis` for
  continuous heatmaps, and `RdBu`/`BrBG`/`PuOr` for centered diverging data.
