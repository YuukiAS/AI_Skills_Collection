...
class SkillRuntimeTextAuditTests(unittest.TestCase):
    ...
    def test_writing_style_phase_two_boundaries(self):
        ...
        self.assertIn("A + B", chinese)
        self.assertIn("A vs B", chinese)
        self.assertIn("A -> B", chinese)
        self.assertIn("noun-stack", chinese)
        self.assertIn("FedFisher", chinese)
        self.assertIn("LoRA", chinese)
        self.assertIn("Dice", chinese)
        self.assertIn("受保护的反向约束", chinese)
        self.assertIn("连字符英文复合名词", chinese)
        self.assertIn("非正式英文 noun-stack", chinese)
        self.assertIn("仓库、manifest、文件状态、实现状态或审计记录", chinese)
        self.assertIn("括号、脚注、方法定位或技术附录", chinese)
        self.assertIn("forensic-level exact proof", chinese)
        self.assertIn("数据使用与字段约束说明", chinese)
        checklist = (REPO_ROOT / "skills/writing/core/chinese-prose/references/chinese-prose-checklist.md").read_text(encoding="utf-8")
        self.assertIn("Classify English before rewriting", checklist)
        self.assertIn("dataset, model, metric", checklist)
        self.assertIn("A + B", checklist)
        self.assertIn("A vs B", checklist)
        self.assertIn("A -> B", checklist)
        self.assertIn("FedFisher, LoRA, and Dice", checklist)
        self.assertIn("repo, manifest, file-status, audit", checklist)
